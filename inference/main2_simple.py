    ...
    def run_inference_on_image(
        ...
        with torch.no_grad():
            try:
                ...
                logger.warning(
                    "MONAI sliding window not available, using direct inference"
                )
    ...
    def process_series(
        ...
        if is_split:
            ...
            if side.lower() == "left":
                ...
                logger.info("Processing both sides - using full image")
        ...
        metadata = {
            ...
            "origin": origin,
            "direction": (
                direction.tolist() if hasattr(direction, "tolist") else list(direction)
            ),
            "is_split": is_split,
...
Examples:
  # Basic usage
  python main2_simple.py --dcm_dir /path/to/dicom --out_dir /path/to/output --model_path /path/to/model.pth

  # With splitting and specific side
  python main2_simple.py --dcm_dir /path/to/dicom --out_dir /path/to/output --model_path /path/to/model.pth --split --side left

  # Custom model name and minimum slices
  python main2_simple.py --dcm_dir /path/to/dicom --out_dir /path/to/output --model_path /path/to/model.pth --model_name CustomUNet --min_slices 15
        """,
    )
