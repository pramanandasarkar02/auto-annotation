...
def keep_bone_only():
    """
    here keep only bone part beacuse i want to work with only bone and remove other part and prepare it for working 3d segmentation
    
    """
def keep_bone_only(combined, logger=None):
    """
    Extract only the bone part from multiclass segmentation for 3D processing.
    Removes all other segments and prepares a clean binary bone mask.
    
    Args:
        combined: 3D array with multiclass segmentation labels (Z × Y × X)
                 Expected labels: 0=Background, 1=Bone, 2=Wrong Bone, 3=Muscle, 4=Extra Part
        logger: Optional logger instance for detailed logging
        
    Returns:
        bone_only: 3D binary array with only bone segment (0=Background, 1=Bone)
    """
    
    if logger:
        logger.info("Starting bone extraction for 3D segmentation...")
    
    # Check input data
    unique_labels = np.unique(combined)
    if logger:
        logger.info(f"Input segmentation contains labels: {unique_labels}")
    
    # Extract only bone (label = 1)
    # Based on your segmentation: 1=Bone, others are different structures
    bone_mask = (combined == 1)
    
    # Create binary bone segmentation
    bone_only = np.zeros_like(combined, dtype=np.uint8)
    bone_only[bone_mask] = 1
    
    # Statistics and validation
    total_voxels = combined.size
    bone_voxels = np.sum(bone_mask)
    bone_percentage = (bone_voxels / total_voxels) * 100
    
    if logger:
        logger.info(f"BONE EXTRACTION RESULTS:")
        logger.info(f"  Total volume: {total_voxels:,} voxels")
        logger.info(f"  Bone voxels: {bone_voxels:,} ({bone_percentage:.2f}%)")
        logger.info(f"  Background voxels: {total_voxels - bone_voxels:,}")
        
        # Check bone connectivity and structure
        logger.info(f"  Bone volume shape: {bone_only.shape} (Z×Y×X)")
        
        # Analyze bone distribution across Z-slices
        z_distribution = []
        for z in range(bone_only.shape[0]):
            slice_bone_count = np.sum(bone_only[z] == 1)
            z_distribution.append(slice_bone_count)
        
        non_empty_slices = sum(1 for count in z_distribution if count > 0)
        max_bone_slice = max(z_distribution)
        avg_bone_per_slice = np.mean([count for count in z_distribution if count > 0]) if non_empty_slices > 0 else 0
        
        logger.info(f"  Z-slice analysis:")
        logger.info(f"    Slices containing bone: {non_empty_slices}/{len(z_distribution)}")
        logger.info(f"    Max bone voxels per slice: {max_bone_slice}")
        logger.info(f"    Avg bone voxels per slice: {avg_bone_per_slice:.1f}")
    
    # Validation checks
    if bone_voxels == 0:
        if logger:
            logger.error("ERROR: No bone voxels found! Check if label '1' represents bone in your segmentation.")
            logger.error("Available labels in input:", unique_labels)
        raise ValueError("No bone segments found. Verify that label '1' corresponds to bone in your segmentation.")
    
    if logger:
        logger.info("✅ Bone extraction completed successfully!")
        logger.info("   Ready for 3D segmentation processing")
...
def convert_nrrd_to_nifti(nrrd_dir, output_dir):
    for patient in os.listdir(nrrd_dir):
        ...
        for series in os.listdir(patient_path):
            ...
            # Find file ending with Femur.nrrd
            # matches = glob.glob(os.path.join(series_path, "*Femur.nrrd"))

            # if matches:
            #     nrrd_path = matches[0]   # take the first match
            #     print("Found:", nrrd_path)
            # else:
            #     print("No Femur.nrrd file found in", series_path)
            ...
            output_path = os.path.join(output_path, f"{patient}_{series.upper()}.nii.gz")
            # convert_nrrd_series(nrrd_path, output_path)
            convert_nrrd_to_nifti_specific(nrrd_path, output_path)
